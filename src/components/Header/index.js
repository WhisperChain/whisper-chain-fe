...
import { createProfile, getProfile, refetchActiveQueries, txIndexed } from "../../utils/lensFunction";
...
const Header = () => {
  ...
  return (<>
            ...
            return (
              ...
              >
                {(() => {
                  if (!connected) {
                    return (
                      <button onClick={signInModalOpenHandler} className={` w-[92px] h-[36px] bg-[#FFFFFF] rounded-[40px] not-italic font-bold text-[#6F1AFF] ${styles.HeaderSignInbtn}`}>
                        Sign In
                      </button>
                    );
                  }
                  ...
                  return window?.localStorage.getItem("profileId") ? (
                    <>
                      <div className="flex justify-center items-center gap-[8px] z-[111] cursor-pointer" onClick={logoutModalOpenHandler}>
                        <div className={`box-border flex justify-center items-center  py-[4px] px-[16px] w-auto h-[36px] bg-[#FFFFFF] rounded-[40px] not-italic font-bold text-[#6F1AFF] ${styles.HeaderSignInbtn}`}>
                      ...
                      <Modal onRequestClose={logoutModalCloseHandler} isOpen={openSignOutModal} style={customStyles} ariaHideApp={false}>
    ...
    >
      ...
      >
        <CustomConnectButton onSignInComplete={signInModalCloseHandler} setCreateProfileModal={setCreateProfileModal} />
        <div
        ...
        >
          ...
          <div className="flex justify-start items-center text-[13px] leading-[160%] font-medium text-[#00000099]">
            New to Lens?
            <CustomConnectButton onSignInComplete={signInModalCloseHandler} setCreateProfileModal={setCreateProfileModal} btnText={"Create an account"} />
          </div>
        </div>
      </div>
    </Modal>
